//연습용 (Ex04_Variable_Scope.java 파일 안에다 클래스 만듬)
...
class Variable{
	...
	4. 생성시점(memory에 로딩되는 시점) : new라는 연산자를 통해서 heap 객체가 만들어 지고 나서 
	*/
	...
	3. 특징 : 접근방법이  >> 클래스이름.static자원	(Variable.cv) >> 객체가 만들어지지 않아도 접근 가능
	                                                          (Math.random();)
	4. 생성시점 : 프로그램이 실행(java hello) 
	        class loader system 에 의해서 클래스 정보를 읽고  (class area, method area) 
	            올리고 나서 올려진 클래스안에 static 자원을 가지고 있다면 static 자원도 같이 memory road 합니다.
	...
	*/
	...
	void method() {
		...
		//생성시점 : 함수가 호출되면 만들어지고 함수가 끝나면 같이 소멸 (stack) 영역
...
public class Ex04_VariableSscope {

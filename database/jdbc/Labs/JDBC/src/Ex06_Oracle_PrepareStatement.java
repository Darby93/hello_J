...
/*
Statement stmt = conn.createStatement(); //생정 주소값을 리턴
stmt.executeQuery("select * from emp where empno = 7788") // 실행

장점 : 쿼리문을 바꾸어 실행이 가능하다 (재사용)
이유 : parameter 없는 경우 > 고정문 쿼리문 (컬러명, 테이블명, 시스템 정보)
...
PreparedStatement (준비된 Statement )
  ...
  (2) 장점 
     ...
     ( 특히, 검색문 ) : query 문장 DB 서버에 보내서 컴파일 ......
  	  실행할때마다 ? (parameter) 전송
...
public class Ex06_Oracle_PrepareStatement {
	...
	public static void main(String[] args) {
		...
		try {
			...
			//SingletonHelper.close(conn);  APP 살아있는 한 존재하게 하려고 ...
		}
